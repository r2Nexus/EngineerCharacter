...
public abstract class BaseCard extends CustomCard {
    ...
    private static String safeCardImage(String path, String cardID) {
        try {
            ...
            if (Gdx.files.internal(MISSING_IMG).exists()) {
                BasicMod.logger.warn("[MISSING CARD IMG] " + cardID + " tried to load: " + path + " -> using " + MISSING_IMG);
                return MISSING_IMG;
            }
    ...
    {
        super(ID, getName(ID), safeCardImage(cardImage, ID), cost, getInitialDescription(ID), cardType, color, rarity, target);
